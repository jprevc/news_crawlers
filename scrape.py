...
def run_spider(spider_name: str) -> list:
    ...
    with open(crawled_output_path) as file:
...
def get_cached_items(cached_items_path: str) -> list:
    ...
    if os.path.exists(cached_items_path):
        with open(cached_items_path) as file:
...
def get_notificator(notificator_type: str, recipients: list):
    ...
    """
    notificator_map = {
        "email": lambda: EmailNotificator(
            recipients, os.environ.get("EMAIL_USER"), os.environ.get("EMAIL_PASS")
        ),
        "pushover": lambda: PushoverNotificator(
            recipients, os.environ.get("PUSHOVER_APP_TOKEN")
        ),
    }
...
if __name__ == "__main__":
    ...
    if new_data:
        ...
        for notificator_type_str, notificator_data in spider_configuration[
            "notifications"
        ].items():
            notificator = get_notificator(
                notificator_type_str, notificator_data["recipients"]
            )

...
class Test_PushoverNotificator(unittest.TestCase):
    ...
    def test_length_of_divided_messages_is_equal_to_original(
        ...
        messages_length_sum = sum(
            len(message) for message in self._session_mock.simulated_messages
        )
    ...
    def test_send_items_separate_correctly_separates_items(
        ...
        self._send_test_items(
            num_test_items, text_length, num_users, send_separately=True
        )

        self.assertEqual(
            num_test_items * num_users, len(self._session_mock.simulated_messages)
        )
...
class Test_EmailNotificator(unittest.TestCase):
    def setUp(self):
        self.email = EmailNotificator(
            ["user_key_1"], "test_email_user", "test_email_pass"
        )
    ...
    def test_send_text(self):
        ...
        self.assertEqual(
            self._smtp_mock.simulated_messages[0],
            b"Subject: test_subject\n\ntest_message",
        )

...
class CarobniSvetSpider(scrapy.Spider):
    ...
    with open(os.path.join(home_path, "carobni_svet_configuration.yaml")) as f:
    ...
    def open_images(self, response) -> scrapy.Request:
        ...
        return scrapy.Request(
            url=self.config_data["urls"]["photos"], callback=self.get_images
        )
    ...
    def get_images(self, response):
        ...
        for item in image_items:
            yield {"type": "image", "url": item.attrib["href"]}

        yield scrapy.Request(
            url=self.config_data["urls"]["blog"],
            callback=self.get_blog,
            dont_filter=True,
        )
    ...
    def get_blog(response):
        ...
        for item in blog_items:
            yield {"type": "blog", "url": item.attrib["id"]}
